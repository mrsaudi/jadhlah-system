...
// جلب بيانات العريس من قاعدة البيانات (بما في ذلك ready_at وcreated_at وpage_views)
$stmt = $pdo->prepare("
    SELECT *
    FROM grooms
    WHERE id = ?
...
// 5. الصفحة نشطة وصالحة للعرض: زيادة عداد الزيارات
$pdo->prepare("
    UPDATE grooms
...
<!-- أيقونة Android Chrome -->
<link rel="icon" type="image/png" sizes="192x192" href="/assets/android-chrome-192x192.png">
<link rel="icon" type="image/png" sizes="512x512" href="/assets/android-chrome-512x512.png">

<!-- أيقونة Apple Touch (iOS) -->
<link rel="apple-touch-icon" sizes="180x180" href="/assets/apple-touch-icon.png">

<!-- ملف Manifest للـ PWA -->
<link rel="manifest" href="/assets/site.webmanifest">

<!-- لون شريط العنوان في بعض المتصفحات -->
<meta name="theme-color" content="#ffc107">
 ...
 <?php
$groomId = (int) $groomData['id'];
$groomName = $groomData['groom_name'];
$eventDate = date('Y/m/d', strtotime($groomData['wedding_date']));
$pageURL = "https://www.jadhlah.com/groom.php?groom=$groomId";
$bannerURL = "https://www.jadhlah.com/grooms/$groomId/modal_thumb/banner.jpg";
$description = "شاهد أجمل لقطات زواج $groomName بتاريخ $eventDate من تصوير جذلة";
?>

<!-- Open Graph Meta Tags -->
<meta property="og:title" content="زواج <?= htmlspecialchars($groomName) ?>">
...
<meta name="twitter:title" content="زواج <?= htmlspecialchars($groomName) ?>">
...
/*  background: #FFF;*/
/*background: linear-gradient(179deg, rgba(255, 255, 255, 1) 0%, rgba(207, 144, 56, 0.61) 31%, rgba(13, 13, 13, 1) 100%);*/
}
...
.download-card .bi-cloud-arrow-down {
  font-size: 3.5rem;
  color: #ffc107;
}
...
<!-- FileSaver.js لحفظ الـ Blob كملف -->
<script
...
<script>
  // قائمة أسماء الملفات كما جاءت في data.json أو من DB
  const imageFiles = <?= json_encode($jsonData['photos'] ?? [], JSON_UNESCAPED_SLASHES) ?>;
  const groomId    = <?= (int)$groomId ?>;
</script>
...
<div class="banner-wrapper">
  <?php if (!empty($groomId)): ?>
    <img src="grooms/<?= $groomId ?>/modal_thumb/banner.jpg?<?= time() ?>" class="banner">
...
<?php if (!empty($groomData['event_name'])): ?>
  <h2 class="text-center mt-3" style="font-size: 1.5rem; font-family: 'Tajawal', sans-serif;">
    <?= htmlspecialchars($groomData['event_name']) ?>
  </h2>
<?php endif; ?>
...
<div class="photo-box"
     ...
     data-id="<?= isset($photo['id']) ? htmlspecialchars($photo['id']) : '' ?>">
      ...
      <div class="modal-scroll">
        <?php foreach ($photos as $photo): ?>
          <div class="modal-photo" data-id="<?= $photo['id'] ?>">
           ...
           <img src="<?= htmlspecialchars("grooms/{$groomId}/modal_thumb/{$photo['filename']}") ?>" loading="lazy" decoding="async">
...
<a href="/download.php?groom=<?= $groomId ?>&file=<?= urlencode($photo['filename']) ?>" 
   download 
   title="تحميل الصورة الأصلية" 
   class="text-yellow-400 text-xl hover:text-yellow-500 transition">
  ⬇️
</a>
...
<!-- زر التحميل -->
<section class="download-section text-center py-5">
  <div class="container">
    <div class="card download-card mx-auto p-4 shadow-lg">
      <div class="card-body">
        <i class="bi bi-cloud-arrow-down display-4 mb-3 text-warning"></i>
        <h5 class="card-title mb-2">أرشف صور حفل <?= htmlspecialchars($groomData['groom_name']) ?></h5>
        <p class="card-text text-muted mb-4">نزّل جميع الصور الأصلية دفعة واحدة دون انتظار الخادم.</p>
        <button id="downloadAllBtn" class="btn btn-gradient btn-lg">
...
<div class="modal fade" id="downloadModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content bg-glass py-4 px-3 rounded-4 border-0">
      <div class="modal-header border-0">
        <h5 class="modal-title w-100 text-center">قيد التحضير...</h5>
      ...
      <div class="modal-body text-center">
        ...
        <span id="downloadPercent" class="d-block mb-2">0%</span>
        <button id="confirmDownload" class="btn btn-sm btn-success me-2">ابدأ التحميل</button>
        <button class="btn btn-sm btn-outline-secondary" data-bs-dismiss="modal">إلغاء</button>
      </div>
    </div>
  </div>
</div>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    // ضمان عرض النافذة دائمًا في البداية (اختياري)
    ...
    // إذا سبق وتم عرضها في الجلسة، لا تظهر مرة أخرى
    if (localStorage.getItem('welcome_shown') === 'yes') {
      welcomeModal.style.display = 'none';
      return;
    }

    // تخفي النافذة تلقائيًا عند تحميل الصفحة بالكامل
    window.addEventListener('load', function () {
      setTimeout(() => {
        welcomeModal.style.opacity = 0;
        setTimeout(() => welcomeModal.style.display = 'none', 500);
        localStorage.setItem('welcome_shown', 'yes');
      }, 1000); // بعد ثانية واحدة من تحميل الصفحة
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const confirmBtn     = document.getElementById('confirmDownload');
  const progressBarEl  = document.getElementById('downloadProgressBar');
  const modalInstance  = new bootstrap.Modal(document.getElementById('downloadModal'));
  const groomId        = <?= json_encode($groomId) ?>;

  // عند النقر على زر التحميل: افتح المودال
  ...
  // عند الموافقة: جمع الصور وضغطها
  confirmBtn.addEventListener('click', async () => {
    downloadBtn.disabled = true;
    const zip = new JSZip();

    // جمع روابط الصور الأصلية وأسمائها
    const items = Array.from(document.querySelectorAll('.photo-box'))
      .map(box => {
        const filename = box.dataset.filename;
        const srcThumb = box.querySelector('img').getAttribute('src');
        const url      = srcThumb.replace('/thumbs/', '/originals/');
        return { filename, url };
      });

    // جلب الصور وإضافتها للـ ZIP مع تحديث الشريط
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        console.warn('فشل تحميل', items[i].url);
      }
